#!/usr/bin/env python3
"""Run multiple London monthly ingests in chronological order."""
...
def main() -> int:
    ...
    ingester = pathlib.Path(__file__).with_name("ingest_london.py")

    print(f"dataSec London backfill: {', '.join(months)}", flush=True)
    for month in months:
